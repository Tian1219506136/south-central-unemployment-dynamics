...
# ===================
# ===================
# Step 1: filter the single state SA csv: Texas
# ===================
# ===================

texas_sa_only <- all_states_both %>%
  filter(state == "Texas", data_type == "SA")

# ===================
# ===================
# Step 2: Texas SA - Unit Root Tests + Differencing Identification for ARIMA
# ===================
# ===================
...
# 1. Prepare Texas SA data
texas_sa_only <- all_states_both %>%
  filter(state == "Texas", data_type == "SA")

m <- 12
y_ts <- ts(texas_sa_only$value, start = c(1995, 1), frequency = m)

# 2. Plot original Texas SA series
plot(y_ts,
     main = "Texas Unemployment Rate (SA)",
...
test_level <- perform_unit_root_tests(as.numeric(y_ts), "Texas SA Level Series")
...
texas_sa_diff1_df <- texas_sa_only %>%
  mutate(diff1 = c(NA, diff(value))) %>%
  filter(!is.na(diff1))

texas_sa_diff1 <- texas_sa_diff1_df$diff1

# 7. Plot first-differenced series
ggplot(texas_sa_diff1_df, aes(x = date, y = diff1)) +
  geom_line() +
  labs(
    title = "Texas SA First Difference",
    x = "Date",
    y = "First Difference"
  )
...
test_diff1 <- perform_unit_root_tests(
  texas_sa_diff1,
  "Texas SA First-Differenced Series"
)

# 9. ACF / PACF on first-differenced series
op <- par(mfrow = c(1, 2))
acf(texas_sa_diff1, lag.max = 36, main = "Texas SA First Difference: ACF")
pacf(texas_sa_diff1, lag.max = 36, main = "Texas SA First Difference: PACF")
par(op)
...
# Texas SA version: chosen working structure d = 1, D = 0
# ===================
# ===================
...
test_working <- perform_unit_root_tests(as.numeric(w_ts), "Texas SA Working Series")

# 6. Plot the differencing progression
op <- par(mfrow = c(2, 2))

plot(y_ts,
     main = "1. Original Texas SA Series",
     ylab = "Level",
     xlab = "Time",
     col = "steelblue",
     lwd = 1.5)
...
acf(w_ts,
    lag.max = 36,
    main = "4. Texas SA Working Series: ACF")
grid(col = "gray80")
...
pacf(w_ts,
     lag.max = 36,
     main = "Texas SA Working Series: PACF")
grid(col = "gray80")
...
# ===================
# ===================
# Step 4: Texas SA ARIMA Estimation
# ===================
# ===================
...
plot(
  forecast_sa_12,
  main = "Texas SA - 12-Month Forecast",
  xlab = "Time",
  ylab = "Texas Unemployment Rate (SA)",
  cex.main = 1
)
...
if (!exists("final_fit")) {
  if (exists("estimation_sa") && is.list(estimation_sa) && !is.null(estimation_sa$best_model)) {
    ...
    stop("No Texas SA fitted model found. Run the SA estimation step first.")
  }
}
...
# ===================
# ===================
# Step 6: Texas SA Diagnostic Checking
# ===================
# ===================
...
if (!exists("final_fit")) {
  if (exists("estimation_sa") && is.list(estimation_sa) && !is.null(estimation_sa$best_model)) {
    ...
    stop("No Texas SA fitted model found. Run the Texas SA estimation step first.")
  }
}
...
plot(resid_vals, type = "l", col = "steelblue",
     main = "1. Texas SA Residuals Over Time",
...
hist(resid_vals, breaks = "FD", freq = FALSE,
     ...
     main = "2. Texas SA Residual Histogram",
     xlab = "Residual")
...
Acf(resid_vals, lag.max = min(48, floor(n/4)),
    main = "3. ACF of Texas SA Residuals",
    col = "steelblue")

# Plot 4: PACF of residuals
Pacf(resid_vals, lag.max = min(48, floor(n/4)),
     main = "4. PACF of Texas SA Residuals",
     col = "steelblue")
...
# Plot 5: Q-Q plot
qqnorm(resid_vals,
       main = "5. Q-Q Plot of Texas SA Residuals",

...
# ===================
# ===================
# Step 1: filter the single state SA csv: Arkansas
# ===================
# ===================

arkansas_sa_only <- all_states_both %>%
  filter(state == "Arkansas", data_type == "SA")

# ===================
# ===================
# Step 2: Arkansas SA - Unit Root Tests + Differencing Identification for ARIMA
# ===================
# ===================
...
# 1. Prepare Arkansas SA data
arkansas_sa_only <- all_states_both %>%
  filter(state == "Arkansas", data_type == "SA")

m <- 12
y_ts <- ts(arkansas_sa_only$value, start = c(1995, 1), frequency = m)

# 2. Plot original Arkansas SA series
plot(y_ts,
     main = "Arkansas Unemployment Rate (SA)",
...
test_level <- perform_unit_root_tests(as.numeric(y_ts), "Arkansas SA Level Series")
...
arkansas_sa_diff1_df <- arkansas_sa_only %>%
  mutate(diff1 = c(NA, diff(value))) %>%
  filter(!is.na(diff1))

arkansas_sa_diff1 <- arkansas_sa_diff1_df$diff1

# 7. Plot first-differenced series
ggplot(arkansas_sa_diff1_df, aes(x = date, y = diff1)) +
  geom_line() +
  labs(
    title = "Arkansas SA First Difference",
    x = "Date",
    y = "First Difference"
  )
...
test_diff1 <- perform_unit_root_tests(
  arkansas_sa_diff1,
  "Arkansas SA First-Differenced Series"
)

# 9. ACF / PACF on first-differenced series
op <- par(mfrow = c(1, 2))
acf(arkansas_sa_diff1, lag.max = 36, main = "Arkansas SA First Difference: ACF")
pacf(arkansas_sa_diff1, lag.max = 36, main = "Arkansas SA First Difference: PACF")
par(op)

# 10. Save identification results
w_ts <- arkansas_sa_diff1
...
# Arkansas SA version: chosen working structure d = 1, D = 0
# ===================
# ===================
...
test_working <- perform_unit_root_tests(as.numeric(w_ts), "Arkansas SA Working Series")

# 6. Plot the differencing progression
op <- par(mfrow = c(2, 2))

plot(y_ts,
     main = "1. Original Arkansas SA Series",
     ylab = "Level",
     xlab = "Time",
     col = "steelblue",
     lwd = 1.5)
...
acf(w_ts,
    lag.max = 36,
    main = "4. Arkansas SA Working Series: ACF")
grid(col = "gray80")
...
pacf(w_ts,
     lag.max = 36,
     main = "Arkansas SA Working Series: PACF")
grid(col = "gray80")
...
# ===================
# ===================
# Step 4: Arkansas SA ARIMA Estimation
# ===================
# ===================
...
plot(
  forecast_sa_12,
  main = "Arkansas SA - 12-Month Forecast",
  xlab = "Time",
  ylab = "Arkansas Unemployment Rate (SA)",
  cex.main = 1
)
...
# =====================
# =====================
# Step 5: Loading Texas SA fitted model
# =====================
# =====================
...
if (!exists("final_fit")) {
  if (exists("estimation_sa") && is.list(estimation_sa) && !is.null(estimation_sa$best_model)) {
    ...
    stop("No Arkansas SA fitted model found. Run the SA estimation step first.")
  }
}
...
# ===================
# ===================
# Step 6: Arkansas SA Diagnostic Checking
# ===================
# ===================
...
# STEP 0: Load fitted Arkansas SA model
if (!exists("final_fit")) {
  if (exists("estimation_sa") && is.list(estimation_sa) && !is.null(estimation_sa$best_model)) {
    final_fit <- estimation_sa$best_model
  } else {
    stop("No Arkansas SA fitted model found. Run the Arkansas SA estimation step first.")
  }
}
...
plot(resid_vals, type = "l", col = "steelblue",
     main = "1. Arkansas SA Residuals Over Time",
...
hist(resid_vals, breaks = "FD", freq = FALSE,
     ...
     main = "2. Arkansas SA Residual Histogram",
     xlab = "Residual")
...
Acf(resid_vals, lag.max = min(48, floor(n/4)),
    main = "3. ACF of Arkansas SA Residuals",
    col = "steelblue")

# Plot 4: PACF of residuals
Pacf(resid_vals, lag.max = min(48, floor(n/4)),
     main = "4. PACF of Arkansas SA Residuals",
     col = "steelblue")
...
# Plot 5: Q-Q plot
qqnorm(resid_vals,
       main = "5. Q-Q Plot of Arkansas SA Residuals",

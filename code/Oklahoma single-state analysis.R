...
# ===================
# ===================
# Step 1: filter the single state SA csv: Oklahoma
# ===================
# ===================

oklahoma_sa_only <- all_states_both %>%
  filter(state == "Oklahoma", data_type == "SA")

# ===================
# ===================
# Step 2: Oklahoma SA - Unit Root Tests + Differencing Identification for ARIMA
# ===================
# ===================
...
# 1. Prepare Oklahoma SA data
oklahoma_sa_only <- all_states_both %>%
  filter(state == "Oklahoma", data_type == "SA")

m <- 12
y_ts <- ts(oklahoma_sa_only$value, start = c(1995, 1), frequency = m)

# 2. Plot original Oklahoma SA series
plot(y_ts,
     main = "Oklahoma Unemployment Rate (SA)",
...
test_level <- perform_unit_root_tests(as.numeric(y_ts), "Oklahoma SA Level Series")
...
oklahoma_sa_diff1_df <- oklahoma_sa_only %>%
  mutate(diff1 = c(NA, diff(value))) %>%
  filter(!is.na(diff1))

oklahoma_sa_diff1 <- oklahoma_sa_diff1_df$diff1

# 7. Plot first-differenced series
ggplot(oklahoma_sa_diff1_df, aes(x = date, y = diff1)) +
  geom_line() +
  labs(
    title = "Oklahoma SA First Difference",
    x = "Date",
    y = "First Difference"
  )
...
test_diff1 <- perform_unit_root_tests(
  oklahoma_sa_diff1,
  "Oklahoma SA First-Differenced Series"
)

# 9. ACF / PACF on first-differenced series
op <- par(mfrow = c(1, 2))
acf(oklahoma_sa_diff1, lag.max = 36, main = "Oklahoma SA First Difference: ACF")
pacf(oklahoma_sa_diff1, lag.max = 36, main = "Oklahoma SA First Difference: PACF")
par(op)

# 10. Save identification results
w_ts <- oklahoma_sa_diff1
...
# Oklahoma SA version: chosen working structure d = 1, D = 0
# ===================
# ===================
...
# 2. Manual selection for Texas SA
D_hat <- 0
d_hat <- 1
...
test_working <- perform_unit_root_tests(as.numeric(w_ts), "Oklahoma SA Working Series")

# 6. Plot the differencing progression
op <- par(mfrow = c(2, 2))

plot(y_ts,
     main = "1. Original Oklahoma SA Series",
     ylab = "Level",
     xlab = "Time",
     col = "steelblue",
     lwd = 1.5)
...
acf(w_ts,
    lag.max = 36,
    main = "4. Oklahoma SA Working Series: ACF")
grid(col = "gray80")
...
pacf(w_ts,
     lag.max = 36,
     main = "Oklahoma SA Working Series: PACF")
grid(col = "gray80")
...
# ===================
# ===================
# Step 4: Oklahoma SA ARIMA Estimation
# ===================
# ===================
...
plot(
  forecast_sa_12,
  main = "Oklahoma SA - 12-Month Forecast",
  xlab = "Time",
  ylab = "Oklahoma Unemployment Rate (SA)",
  cex.main = 1
)
...
# =====================
# =====================
# Step 5: Loading Oklahoma SA fitted model
# =====================
# =====================
...
if (!exists("final_fit")) {
  if (exists("estimation_sa") && is.list(estimation_sa) && !is.null(estimation_sa$best_model)) {
    ...
    stop("No Oklahoma SA fitted model found. Run the SA estimation step first.")
  }
}
...
# ===================
# ===================
# Step 6: Oklahoma SA Diagnostic Checking
# ===================
# ===================
...
# STEP 0: Load fitted Oklahoma SA model
if (!exists("final_fit")) {
  if (exists("estimation_sa") && is.list(estimation_sa) && !is.null(estimation_sa$best_model)) {
    final_fit <- estimation_sa$best_model
  } else {
    stop("No Oklahoma SA fitted model found. Run the Oklahoma SA estimation step first.")
  }
}
...
plot(resid_vals, type = "l", col = "steelblue",
     main = "1. Oklahoma SA Residuals Over Time",
...
hist(resid_vals, breaks = "FD", freq = FALSE,
     ...
     main = "2. Oklahoma SA Residual Histogram",
     xlab = "Residual")
...
Acf(resid_vals, lag.max = min(48, floor(n/4)),
    main = "3. ACF of Oklahoma SA Residuals",
    col = "steelblue")

# Plot 4: PACF of residuals
Pacf(resid_vals, lag.max = min(48, floor(n/4)),
     main = "4. PACF of Oklahoma SA Residuals",
     col = "steelblue")
...
# Plot 5: Q-Q plot
qqnorm(resid_vals,
       main = "5. Q-Q Plot of Oklahoma SA Residuals",

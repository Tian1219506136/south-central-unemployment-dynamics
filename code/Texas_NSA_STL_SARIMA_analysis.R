...
# ===================
# ===================
# Step 1: filter the single state NSA csv: Texas
# ===================
# ===================

texas_nsa_only <- all_states_both %>%
  filter(state == "Texas", data_type == "NSA")

# ===================
# ===================
# Step 2: Texas NSA: STL + Unit Root Tests + Differencing Identification for SARIMA
# =================
# ===================
...
m <- 12
y_ts <- ts(texas_nsa_only$value, start = c(1995, 1), frequency = m)
# 2. STL decomposition
texas_nsa_stl <- stl(y_ts, s.window = "periodic")
plot(texas_nsa_stl, main = "STL Decomposition of Texas Unemployment Rate (NSA)")

texas_stl_df <- data.frame(
  date = texas_nsa_only$date,
  observed = as.numeric(y_ts),
  seasonal = as.numeric(texas_nsa_stl$time.series[, "seasonal"]),
  trend = as.numeric(texas_nsa_stl$time.series[, "trend"]),
  remainder = as.numeric(texas_nsa_stl$time.series[, "remainder"])
)
...
test_level <- perform_unit_root_tests(as.numeric(y_ts), "Texas NSA Level Series")
...
texas_nsa_diff1_df <- texas_nsa_only %>%
  mutate(diff1 = c(NA, diff(value))) %>%
  filter(!is.na(diff1))
texas_nsa_diff1 <- texas_nsa_diff1_df$diff1

   # B. Seasonal difference only (lag 12)
texas_nsa_diff12_df <- texas_nsa_only %>%
  mutate(diff12 = value - lag(value, 12)) %>%
  filter(!is.na(diff12))
texas_nsa_diff12 <- texas_nsa_diff12_df$diff12

   # C. First + seasonal difference
texas_nsa_diff1_diff12_df <- texas_nsa_diff1_df %>%
  mutate(diff1_diff12 = diff1 - lag(diff1, 12)) %>%
  filter(!is.na(diff1_diff12))

texas_nsa_diff1_diff12 <- texas_nsa_diff1_diff12_df$diff1_diff12
# 7. Plot transformed series
ggplot(texas_nsa_diff1_df, aes(x = date, y = diff1)) +
  geom_line() +
  labs(
    title = "Texas NSA First Difference",
    x = "Date",
    y = "First Difference"
  )

ggplot(texas_nsa_diff12_df, aes(x = date, y = diff12)) +
  geom_line() +
  labs(
    title = "Texas NSA Seasonal Difference (lag 12)",
    x = "Date",
    y = "Seasonal Difference"
  )

ggplot(texas_nsa_diff1_diff12_df, aes(x = date, y = diff1_diff12)) +
  geom_line() +
  labs(
    title = "Texas NSA First + Seasonal Difference",
    x = "Date",
    y = "First + Seasonal Difference"
  )
# 8. Unit root tests on transformed series
test_diff1 <- perform_unit_root_tests(texas_nsa_diff1, "Texas NSA First-Differenced Series")
test_diff12 <- perform_unit_root_tests(texas_nsa_diff12, "Texas NSA Seasonal-Differenced Series (lag 12)")
test_diff1_diff12 <- perform_unit_root_tests(
  texas_nsa_diff1_diff12,
  "Texas NSA First + Seasonal-Differenced Series"
)
# 9. ACF / PACF comparison
op <- par(mfrow = c(1, 2))
acf(texas_nsa_diff1, lag.max = 36, main = "Texas NSA First Difference: ACF")
pacf(texas_nsa_diff1, lag.max = 36, main = "Texas NSA First Difference: PACF")
par(op)

op <- par(mfrow = c(1, 2))
acf(texas_nsa_diff12, lag.max = 36, main = "Texas NSA Seasonal Difference: ACF")
pacf(texas_nsa_diff12, lag.max = 36, main = "Texas NSA Seasonal Difference: PACF")
par(op)

op <- par(mfrow = c(1, 2))
acf(texas_nsa_diff1_diff12, lag.max = 36, main = "Texas NSA First + Seasonal Difference: ACF")
pacf(texas_nsa_diff1_diff12, lag.max = 36, main = "Texas NSA First + Seasonal Difference: PACF")
par(op)
...
# 2. Manual selection for Texas NSA
...
if (D_hat > 0) {
  test_seas <- perform_unit_root_tests(as.numeric(y_unseas), "Texas NSA Seasonally Differenced Series")
}
...
test_working <- perform_unit_root_tests(as.numeric(w_ts), "Texas NSA Working Series")

# 6. Plot the differencing progression
op <- par(mfrow = c(2, 2))

plot(y_ts,
     main = "1. Original Texas NSA Series",
     ylab = "Level",
     xlab = "Time",
     col = "steelblue",
     lwd = 1.5)
...
Acf(w_ts, lag.max = 24,
    main = "Texas NSA Working Series: ACF (Non-seasonal Focus)",
    col = "steelblue")

Pacf(w_ts, lag.max = 24,
     main = "Texas NSA Working Series: PACF (Non-seasonal Focus)",
     col = "steelblue")

Acf(w_ts, lag.max = 72,
    main = "Texas NSA Working Series: ACF (Seasonal Lags)",
    col = "darkgreen")

Pacf(w_ts, lag.max = 72,
     main = "Texas NSA Working Series: PACF (Seasonal Lags)",
     col = "darkgreen")
...
# ===================
# ===================
# Step 4 : Texas NSA SARIMA Estimation
# ===================
# ===================
...
plot(forecast_nsa_12,
     ...
     xlab = "Time",
     ylab = "Texas Unemployment Rate (NSA)",
     cex.main = 1
     )
# 7. Save results
estimation_nsa <- list(
...
# =====================
# =====================
# Step 5: Loading Texas NSA fitted model
# =====================
# =====================
...
if (!exists("final_fit")) {
  if (exists("estimation_nsa") && is.list(estimation_nsa) && !is.null(estimation_nsa$best_model)) {
    ...
    stop("No Texas NSA fitted model found. Run the NSA estimation step first.")
  }
}
...
if (!exists("final_fit")) {
  if (exists("estimation_nsa") && is.list(estimation_nsa) && !is.null(estimation_nsa$best_model)) {
    ...
    stop("No Texas NSA fitted model found. Run the Texas NSA estimation step first.")
  }
}
...
plot(resid_vals, type = "l", col = "steelblue",
     main = "1. Texas NSA Residuals Over Time",
...
hist(resid_vals, breaks = "FD", freq = FALSE,
     ...
     main = "2. Texas NSA Residual Histogram",
     xlab = "Residual")
...
Acf(resid_vals, lag.max = min(48, floor(n/4)),
    main = "3. ACF of Texas NSA Residuals",
    col = "steelblue")

   # Plot 4: PACF of residuals
Pacf(resid_vals, lag.max = min(48, floor(n/4)),
     main = "4. PACF of Texas NSA Residuals",
     col = "steelblue")
...
par(mfrow = c(1, 2))
   ...
   # Plot 5: Q-Q plot
qqnorm(resid_vals,
       main = "5. Q-Q Plot of Texas NSA Residuals",

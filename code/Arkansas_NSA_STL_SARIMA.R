...
# ===================
# ===================
# Step 1: filter the single state NSA csv: Arkansas
# ===================
# ===================

arkansas_nsa_only <- all_states_both %>%
  filter(state == "Arkansas", data_type == "NSA")

# ===================
# ===================
# Step 2: Arkansas NSA: STL + Unit Root Tests + Differencing Identification for SARIMA
# =================
# ===================
...
library(ggplot2)
library(forecast)
# 1. Prepare Arkansas NSA data
arkansas_nsa_only <- all_states_both %>%
  filter(state == "Arkansas", data_type == "NSA")

m <- 12
y_ts <- ts(arkansas_nsa_only$value, start = c(1995, 1), frequency = m)
# 2. STL decomposition
arkansas_nsa_stl <- stl(y_ts, s.window = "periodic")
plot(arkansas_nsa_stl, main = "STL Decomposition of Arkansas Unemployment Rate (NSA)")

arkansas_stl_df <- data.frame(
  date = arkansas_nsa_only$date,
  observed = as.numeric(y_ts),
  seasonal = as.numeric(arkansas_nsa_stl$time.series[, "seasonal"]),
  trend = as.numeric(arkansas_nsa_stl$time.series[, "trend"]),
  remainder = as.numeric(arkansas_nsa_stl$time.series[, "remainder"])
)

head(arkansas_stl_df)
...
test_level <- perform_unit_root_tests(as.numeric(y_ts), "Arkansas NSA Level Series")
...
arkansas_nsa_diff1_df <- arkansas_nsa_only %>%
  mutate(diff1 = c(NA, diff(value))) %>%
  filter(!is.na(diff1))
arkansas_nsa_diff1 <- arkansas_nsa_diff1_df$diff1

# B. Seasonal difference only (lag 12)
arkansas_nsa_diff12_df <- arkansas_nsa_only %>%
  mutate(diff12 = value - lag(value, 12)) %>%
  filter(!is.na(diff12))
arkansas_nsa_diff12 <- arkansas_nsa_diff12_df$diff12

# C. First + seasonal difference
arkansas_nsa_diff1_diff12_df <- arkansas_nsa_diff1_df %>%
  mutate(diff1_diff12 = diff1 - lag(diff1, 12)) %>%
  filter(!is.na(diff1_diff12))

arkansas_nsa_diff1_diff12 <- arkansas_nsa_diff1_diff12_df$diff1_diff12
# 7. Plot transformed series
ggplot(arkansas_nsa_diff1_df, aes(x = date, y = diff1)) +
  geom_line() +
  labs(
    title = "Arkansas NSA First Difference",
    x = "Date",
    y = "First Difference"
  )

ggplot(arkansas_nsa_diff12_df, aes(x = date, y = diff12)) +
  geom_line() +
  labs(
    title = "Arkansas NSA Seasonal Difference (lag 12)",
    x = "Date",
    y = "Seasonal Difference"
  )

ggplot(arkansas_nsa_diff1_diff12_df, aes(x = date, y = diff1_diff12)) +
  geom_line() +
  labs(
    title = "Arkansas NSA First + Seasonal Difference",
    x = "Date",
    y = "First + Seasonal Difference"
  )
# 8. Unit root tests on transformed series
test_diff1 <- perform_unit_root_tests(arkansas_nsa_diff1, "Arkansas NSA First-Differenced Series")
test_diff12 <- perform_unit_root_tests(arkansas_nsa_diff12, "Arkansas NSA Seasonal-Differenced Series (lag 12)")
test_diff1_diff12 <- perform_unit_root_tests(
  arkansas_nsa_diff1_diff12,
  "Arkansas NSA First + Seasonal-Differenced Series"
)
# 9. ACF / PACF comparison
op <- par(mfrow = c(1, 2))
acf(arkansas_nsa_diff1, lag.max = 36, main = "Arkansas NSA First Difference: ACF")
pacf(arkansas_nsa_diff1, lag.max = 36, main = "Arkansas NSA First Difference: PACF")
par(op)

op <- par(mfrow = c(1, 2))
acf(arkansas_nsa_diff12, lag.max = 36, main = "Arkansas NSA Seasonal Difference: ACF")
pacf(arkansas_nsa_diff12, lag.max = 36, main = "Arkansas NSA Seasonal Difference: PACF")
par(op)

op <- par(mfrow = c(1, 2))
acf(arkansas_nsa_diff1_diff12, lag.max = 36, main = "Arkansas NSA First + Seasonal Difference: ACF")
pacf(arkansas_nsa_diff1_diff12, lag.max = 36, main = "Arkansas NSA First + Seasonal Difference: PACF")
par(op)
...
# 2. Manual selection for Arkansas NSA
...
if (D_hat > 0) {
  test_seas <- perform_unit_root_tests(as.numeric(y_unseas), "Arkansas NSA Seasonally Differenced Series")
}
...
test_working <- perform_unit_root_tests(as.numeric(w_ts), "Arkansas NSA Working Series")

# 6. Plot the differencing progression
op <- par(mfrow = c(2, 2))

plot(y_ts,
     main = "1. Original Arkansas NSA Series",
     ylab = "Level",
     xlab = "Time",
     col = "steelblue",
     lwd = 1.5)
...
Acf(w_ts, lag.max = 24,
    main = "Arkansas NSA Working Series: ACF (Non-seasonal Focus)",
    col = "steelblue")

Pacf(w_ts, lag.max = 24,
     main = "Arkansas NSA Working Series: PACF (Non-seasonal Focus)",
     col = "steelblue")

Acf(w_ts, lag.max = 72,
    main = "Arkansas NSA Working Series: ACF (Seasonal Lags)",
    col = "darkgreen")

Pacf(w_ts, lag.max = 72,
     main = "Arkansas NSA Working Series: PACF (Seasonal Lags)",
     col = "darkgreen")
...
# ===================
# ===================
# Step 4 : Arkansas NSA SARIMA Estimation
# ===================
# ===================
...
plot(forecast_nsa_12,
     ...
     xlab = "Time",
     ylab = "Arkansas Unemployment Rate (NSA)",
     cex.main = 1
)
# 7. Save results
estimation_nsa <- list(
...
# =====================
# =====================
# Step 5: Loading Arkansas NSA fitted model
# =====================
# =====================
...
if (!exists("final_fit")) {
  if (exists("estimation_nsa") && is.list(estimation_nsa) && !is.null(estimation_nsa$best_model)) {
    ...
    stop("No Arkansas NSA fitted model found. Run the NSA estimation step first.")
  }
}
...
if (!exists("final_fit")) {
  if (exists("estimation_nsa") && is.list(estimation_nsa) && !is.null(estimation_nsa$best_model)) {
    ...
    stop("No Arkansas NSA fitted model found. Run the Arkansas NSA estimation step first.")
  }
}
...
plot(resid_vals, type = "l", col = "steelblue",
     main = "1. Arkansas NSA Residuals Over Time",
...
hist(resid_vals, breaks = "FD", freq = FALSE,
     ...
     main = "2. Arkansas NSA Residual Histogram",
     xlab = "Residual")
...
Acf(resid_vals, lag.max = min(48, floor(n/4)),
    main = "3. ACF of Arkansas NSA Residuals",
    col = "steelblue")

# Plot 4: PACF of residuals
Pacf(resid_vals, lag.max = min(48, floor(n/4)),
     main = "4. PACF of Arkansas NSA Residuals",
     col = "steelblue")
...
# Plot 5: Q-Q plot
qqnorm(resid_vals,
       main = "5. Q-Q Plot of Arkansas NSA Residuals",
